                                ...
                                for script, infos in script2infos.items():
                                    for info in infos:
                                        if len(info) >= 15:
                                            ...
                                            clean_source_one = ''.join(func_str_one.strip().split()).replace(';', '') 
                                            clean_source_two = ''.join(func_str_two.strip().split()).replace(';', '')
                                            if clean_source_one == clean_source_two:
                                                if category not in cat2rank2target2infos:
                                                    cat2rank2target2infos[category] = dict()
                                                if rank not in cat2rank2target2infos[category]:
                                                    cat2rank2target2infos[category][rank] = dict()
                                                if target not in cat2rank2target2infos[category][rank]:
                                                    cat2rank2target2infos[category][rank][target] = list()
                                                cnt += 1
                                                cat2rank2target2infos[category][rank][target].append(info)
                                            ...
                                            try:
                                                ...
                                                continue
                                            try:
                                                program_one = es5(unicode(source_one))
                                            except Exception as e:
                                                #print(e)
                                                continue
                                            function_source_one = None
                                            walker = Walker()
                                            for node in walker.filter(program_one, lambda node:(isinstance(node, FuncDecl))):
                                                if str(node.identifier) == str(target):
                                                    function_source_one = str(node)
                                                    break

                                            try:
                                                program_two = es5(unicode(source_two))
                                            except Exception as e:
                                                #print(e)
                                                continue
                                            function_source_two = None
                                            walker = Walker()
                                            for node in walker.filter(program_two, lambda node:(isinstance(node, FuncDecl))):
                                                if str(node.identifier) == str(target):
                                                    function_source_two = str(node)
                                                    break
                                                
                                            if function_source_one is not None and function_source_two is not None:
                                                #print(rank, program_one, program_two)
                                                #print(rank)
                                                clean_source_one = ''.join(function_source_one.strip().split()).replace(';', '') 
                                                clean_source_two = ''.join(function_source_two.strip().split()).replace(';', '')
                                                if clean_source_one == clean_source_two:
                                                    if category not in cat2rank2target2infos:
                                                        cat2rank2target2infos[category] = dict()
                                                    if rank not in cat2rank2target2infos[category]:
