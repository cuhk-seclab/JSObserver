...
def measure(user_dir, task_id, start, end):
    ...
    files = [f for f in files if f.endswith('-used-category2type2target2infos.json')]
    #print(files)
    for f in files:
        try:
            rank = f.split('.')[0]
            ...
            with open(input_file, 'r') as input_f:
                category2type2target2infos = json.loads(input_f.read())

                for category, type2target2infos in category2type2target2infos.items():
...
def main(argv):
    ...
    output_file = 'used-category2rank2target2type2infos.json'
